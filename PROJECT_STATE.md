...
- **Réunions §30 (DoD#2) — LANDÉ + LIVE 2026-06-06 (`origin/main`=`3689756`)** : ✅ axe « présence réunions » entièrement construit : #207 J-M1 data (migration `20260530150000` prod) · #243 J-M2 `/reunions` member · #244 J-M3 admin (cron + `/admin/reunions` + `?tab=presence` + slugs `meeting.generated`/`admin.meeting.cancelled`) · #245 J-M4 engagement (`WEIGHT_MEETING` additif, byte-identique, firewall green). Les 6 PRs du backlog (#200/#201/#202/#204/#205/#206) déjà landées (cf. MAJ-2). ⚠️ **Ops restant** : `scp ops/cron/crontab.fxmily` → serveur `/etc/cron.d/fxmily-app` (le cron n'est PAS auto-déployé par deploy.yml — sinon 0 réunion générée). **Reste S2 (vrai 100%)** : (1) wiring crontab ; (2) **scoring DoD#3** — `lib/scoring` consomme `marketAnalysisDone` / émotions trade avant-pendant-après / sommeil en sous-scores ADDITIFS null-skip (pattern J-M4) ; (3) **`/spec` suivi-formation** (axe §28, interactif Eliot) ; (4) vérif DoD §29 e2e complète.

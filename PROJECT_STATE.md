...
- **Backlog PR S2 ouvert (2026-06-06, `gh pr list` + `git merge-tree` vs main) — à lander/rebaser** : 🟢 CLEAN #200 (tests routes onboarding-batch) · #202 (a11y dup `id=main-content`) · #205 (e2e de-flake CI) → `gh pr update-branch`+CI+squash-merge (branch-protection **`strict:true`** → sérialisé). 🟠 conflit code #201 (validate answers vs instrument — chevauche #240 sur `onboarding-interview/service.ts`) · #207/#208 (réunions, `schema.prisma`). 🟡 conflit doc seul #204 (JWT revocation, **sécu — revue**) · #206 (spec). **Réunions §30 = J-M1 (#207)/J-M2 (#208) seulement ; J-M3 (admin) + J-M4 (engagement) NON construits → `model Meeting` absent de main → DoD#2 non fermé (multi-jalon).**

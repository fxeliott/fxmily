...
>
> **Snapshot** : 2026-06-05 · `origin/main` = `5dce847` · Mainteneur Session 1 : Claude Code (Opus 4.8) · Auteur produit : Eliot Pena.
...
| **2**   | **Onboarding & Profil IA** (V2.4 §L)                         | ✅ LIVE prod       | `OnboardingInterview(+Answer)`, `MemberProfile`                                                    | `/onboarding/interview*`, `/profile`, **`/dashboard` (widget)**            | ✅ **Pipeline branché sur le parcours** (PR profile-widget — était LIVE mais orphelin, 0 lien dashboard). Reste : re-profilage périodique ; dette `questionText:''` ; allowlist `opus-4-8` ; smoke batch data réelle. |

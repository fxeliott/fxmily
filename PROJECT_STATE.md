...
>
> **Snapshot** : 2026-06-05 · `origin/main` = `929cfdc` (Session 2 jalons mergés+déployés LIVE : `#235` profil widget `d43ae2b` + `#236` émotion-pendant `929cfdc`, migration `20260605114211_add_trade_emotion_during` appliquée prod, health ok/ok/ok) · Mainteneur Session 1 : Claude Code (Opus 4.8) · Auteur produit : Eliot Pena.
...
- **Profilage IA — dettes (audit Session 2, à traiter en jalons dédiés)** : (a) `OnboardingInterviewAnswer.questionText` persisté à `''` ([`service.ts:217,225`], commentaire « Phase A.2 » jamais réalisé ; le batch retombe sur `item.text`, OK aujourd'hui mais fragile cross-version) → populer depuis `instrument-v1` au write. (b) `env.ANTHROPIC_MODEL` allowlist exclut `claude-opus-4-8` ([`env.ts:89`], défaut `sonnet-4-6`) → le path TS (mock/live/fallback) n'est pas aligné §28 ; le path bash local prod est correct. (c) re-profilage périodique (pipeline strictement one-shot `@unique userId`).
...
**Session 2 — FINALISÉE (2 jalons LIVE prod)** : (1) **PR #235** widget profil **MERGÉ `d43ae2b`** + deploy Hetzner SUCCESS + prod ok/ok/ok → le profilage est découvrable. (2) **PR #236** **émotion PENDANT le trade** (§22 « avant/pendant/après ») **MERGÉ `929cfdc`** — `Trade.emotionDuring`, migration additive sûre **appliquée prod** [deploy log « All migrations have been successfully applied »], capture+affichage+rapport IA hebdo, gate vert (Vitest **1768**), e2e réel **4/4** (full form submit), **4 audits Opus** (security 0 Crit/High/Med + verifier 7/7) + complétion audit-driven (agrégation IA). **Accounting honnête §29.2** : 6 axes captés + émotion-pendant LIVE ; **réunions §30 EN CONSTRUCTION** (#206 spec + #207 data + #208 UI, non mergées) ; **suivi-formation déféré** (`docs/defer-suivi-formation-jalon4`) ; **analyse-marché déclarée non démarrée** (prochain axe buildable seul). **CANON** : survoler `gh pr list` + `git branch -a` AVANT d'auditer « ce qui manque » (mon audit scopé `main` avait raté ~9 PRs feature en vol).

...
- **Stats = calculées, jamais stockées** : agrégées au render depuis `TrainingTrade` (`enteredAt` dans `[weekStart, weekEnd]`, champs `systemRespected`/`pair`/`lessonLearned` — **JAMAIS `resultR`/`outcome`**) + count `TrainingAnnotation` de la semaine. Pure aggregation, recompute-safe.
...
- **Erreurs** : Zod `safeParse` du payload complet côté serveur (autorité) ; `weekStart` doit être un lundi UTC dans une fenêtre bornée `[-35 j, +7 j]` ; `weekEnd` service-computed (SSOT anti-tamper) ; injection suspectée → audit metadata + Sentry warning, **jamais bloquant** (un FP ne doit pas manger le texte membre).
...
5. Découpage suggéré (jalon atomique, ~12-16 fichiers) : **backend** (migration `TrainingDebrief` + Zod `trainingDebriefSchema` + service user-scoped + stats aggregator pur + Server Action carbone REFLECT + audit slugs + tests TDD) → STOP/confirm (backend-first canon) → **frontend** (`/training/debrief` landing + `/training/debrief/new` wizard cyan + panneau stats + admin read-only dans l'onglet training existant + Playwright auth-gates + happy-path). 1 PR atomic.

...
- [ ] **Round-trip DST** : `date === localDateOf(scheduledAt, 'Europe/Paris')` vérifié sur les **2 jours de bascule 2026-03-29 (spring-forward) et 2026-10-25 (fall-back)** + 20h évening (pas seulement 12h).
...
- **J-M1 — Data layer** : 3 enums + 2 models `Meeting`/`MeetingAttendance` + migration (`prisma-migration-runner`) + **réutiliser le helper DST réel `localInstantToUtc` de `lib/weekly-report/week-window.ts:138`** (wall-clock Paris → instant UTC, déjà testé ; **NE PAS** recréer ni importer `datetime-paris.ts` = supprimé PR #172, ni `localMinutes` = non inversible) + Zod schemas + service pur (`countMeetingAttendance` → `{scheduledCount, completedCount, lastDeclaredAt}` + calcul taux, branche `insufficient_data` honnête carbone `analytics.test.ts:34/240-259`) + tests TDD (boundary, **DST jours de bascule 2026-03-29 / 2026-10-25**, round-trip `date ↔ scheduledAt`, idempotence). Backend-only.

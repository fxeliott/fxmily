...
- Suivi-formation/cursus (#4) → **DIFFÉRÉ — post-projet pédagogie** (décision Eliot 2026-05-19 — le contenu pédagogique du cursus Fxmily est en cours de production dans un projet upstream séparé, livraison annoncée « dans quelques semaines » ; écrire la spec de l'app de suivi avant l'existence de la structure pédagogique = inventer le domaine coach). Cadrage `/spec` dédié relancé une fois le projet pédagogie publié ; séquence §21.6 reste à 4 jalons (verrouillée 2026-05-18, gap-analysis 2026-05-17) : #1 #2 #3 CLOS+déployés (#132 / #135 / #137) ; #4 bloqué upstream — un `/spec` + un build chacun, jamais bundlés §18.4.

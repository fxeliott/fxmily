# Journal des jalons livres

> Deplace depuis `CLAUDE.md` le 2026-07-25. Contenu inchange, aucune ligne perdue :
> les 4333 lignes deplacees sont octet pour octet identiques a l'original (verifie).
>
> Raison : un CLAUDE.md de sous-repertoire est recharge EN ENTIER a chaque lecture de fichier
> du dossier. Ce journal pesait ~103 000 tokens par rechargement et saturait le contexte des
> sessions. Il reste consultable ici, et Claude le lit a la demande.
>
> ATTENTION — corruption d'encodage heritee, presente des l'ecriture du texte (commit
> `f7213917`) et donc anterieure a ce deplacement : un interpreteur d'echappements C a
> consomme des sequences antislash + lettre, laissant 22 octets de controle (BEL, VT, FF,
> NUL) et des identifiants amputes de leur premiere lettre — `imingSafeEqual` pour
> `timingSafeEqual`, `evalidatePath` pour `revalidatePath`, `ole="group"` pour `role="group"`,
> `pp/api` pour `app/api`. Conservee TELLE QUELLE ici pour que le deplacement reste prouvable
> octet pour octet. La reparer est un travail separe : les cas ou la lettre avalee n'a laisse
> aucune trace scannable ne se trouvent qu'a la relecture, pas au scan.

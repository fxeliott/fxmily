...
| `EmptyState`                | `@/components/ui/empty-state`     | 6 strates : `icon`(`Target`)/`headline`/`lead`/`guides[]`/`tip`/`ctaPrimary`+`ctaHref`(`<Link>`)/`onPrimary`/`headingLevel`(`'h2'`)                                       | `empty-state.tsx:8-43`      |
...
| `Sparkline`                 | `@/components/ui/sparkline`       | client, 0-dep SVG ; `data`(req, null si <2) ; `width`140/`height`36/`color``var(--acc)`/`fill`/`showLastDot`/`animate`/`ariaLabel`                                        | `sparkline.tsx:8-30`        |

...
/**
 * Segment error boundary for `/admin` (Next.js `error.tsx` convention).
 *
 * Unlike `app/error.tsx` (the GLOBAL boundary, which replaces the whole
 * document), a segment boundary renders BELOW the AppShell — so the admin nav
 * group, ⌘K palette and footer stay mounted. A `throw` from a force-dynamic
 * admin aggregation (e.g. a transient DB hiccup) degrades gracefully into a calm
 * panel inside the console chrome, instead of falling back to the generic root
 * boundary. Mirrors `app/dashboard/error.tsx`.
 */
    ...
    <div className="mx-auto flex w-full max-w-xl flex-1 flex-col items-center justify-center gap-6 px-4 py-12 text-center">
      ...
      <div>
        ...
        <h1 className="mt-2 text-2xl font-semibold tracking-tight text-[var(--t-1)] sm:text-3xl">
          La console admin n&apos;a pas pu s&apos;afficher
        </h1>
        <p className="mt-3 text-sm leading-relaxed text-[var(--t-2)]">
          Rien de grave, on a logué l&apos;erreur côté serveur. Aucune donnée membre n&apos;est
          touchée. Tu peux réessayer cette page, ou revenir à l&apos;accueil. Si ça persiste,
          contacte{' '}
          <a

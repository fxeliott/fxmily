...
/**
 * J-E (expansion IA §21.5) — Tests for POST /api/admin/member-profile-batch/pull.
 * Carbon of the monthly-batch pull route test.
 */
...
vi.mock('@/lib/member-profile-monthly/batch', () => ({
  loadAllReprofileSnapshots: loadMock,
}));
...
function makeRequest(opts: { token?: string; ip: string; query?: string }): Request {
  const url = `https://app.fxmilyapp.com/api/admin/member-profile-batch/pull${opts.query ?? ''}`;
...
describe('GET /api/admin/member-profile-batch/pull', () => {
...
describe('POST /api/admin/member-profile-batch/pull — auth gate', () => {
  it('returns 401 when X-Admin-Token header is missing', async () => {
    const res = await POST(makeRequest({ ip: '10.80.1.1' }) as never);
  ...
  it('returns 401 when X-Admin-Token header is wrong', async () => {
    const res = await POST(
      makeRequest({ token: 'WRONG' + 'a'.repeat(60), ip: '10.80.1.2' }) as never,
    );
    expect(res.status).toBe(401);
    expect(loadMock).not.toHaveBeenCalled();
  });
...
describe('POST /api/admin/member-profile-batch/pull — happy path', () => {
  it('returns the envelope as JSON when token is valid (default = just-ended month)', async () => {
    ...
    const res = await POST(makeRequest({ token: TEST_TOKEN, ip: '10.80.1.3' }) as never);
  ...
  it('forwards currentMonth: true when ?currentMonth=true', async () => {
    ...
    await POST(
      makeRequest({ token: TEST_TOKEN, ip: '10.80.1.4', query: '?currentMonth=true' }) as never,
    );
    expect(loadMock).toHaveBeenCalledWith({ currentMonth: true });
  });
});

describe('POST /api/admin/member-profile-batch/pull — error path', () => {
  it('returns 500 + reportError when loader throws', async () => {
    loadMock.mockRejectedValueOnce(new Error('db_unreachable'));

    const res = await POST(makeRequest({ token: TEST_TOKEN, ip: '10.80.1.5' }) as never);
    expect(res.status).toBe(500);
    expect(reportErrorMock).toHaveBeenCalledTimes(1);
  });
});

describe('POST /api/admin/member-profile-batch/pull — MEMBER_LABEL_SALT prod guard', () => {
  it('returns 503 in production NODE_ENV when MEMBER_LABEL_SALT is unset', async () => {
    ...
    const res = await gated(makeRequest({ token: TEST_TOKEN, ip: '10.80.1.6' }) as never);

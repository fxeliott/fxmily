...
import { runMonthlyDebriefOverdueAlert } from '@/lib/monthly-debrief/overdue';
import { flushSentry, reportError } from '@/lib/observability';
import { callerIdTrusted, cronLimiter } from '@/lib/rate-limit/token-bucket';

/**
 * Cron endpoint — §25 monthly debrief overdue ADMIN nudge (Session 5, DoD#2).
 *
 * Read-only safety-net for the manual monthly debrief batch. The batch is
 * triggered by hand (ban-risk human-in-the-loop §5.4) ; this cron makes that
 * design RELIABLE by nudging the admin when the last completed month's member
 * debriefs are missing past the grace window — so no member silently loses
 * their monthly debrief (SPEC §25.4). It never drives Claude : it only counts
 * rows and emails the operator. See {@link runMonthlyDebriefOverdueAlert}.
 *
 * Wiring expected in production : Hetzner crontab daily 11:10 UTC (13:10 Paris) →
 *   curl -fsS -X POST -H "X-Cron-Secret: $CRON_SECRET" \
 *        https://app.fxmilyapp.com/api/cron/monthly-debrief-overdue-alert
 *
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    reportError('cron.monthly-debrief-overdue-alert', err, {
      route: '/api/cron/monthly-debrief-overdue-alert',
    });
    await flushSentry();

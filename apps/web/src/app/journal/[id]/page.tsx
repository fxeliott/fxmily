        ...
        <section className="bg-card flex flex-col gap-3 rounded-lg border border-[var(--border)] p-4">
          ...
          {trade.realizedRSource === 'estimated' ? (
            <p className="text-muted text-xs">
              Valeur estimée — ajoute un stop-loss à tes trades pour un R réalisé exact.
            </p>
          ) : null}
        </section>
      ) : null}
            ...
            <section className="bg-card flex flex-col gap-3 rounded-lg border border-[var(--border)] p-4">
              ...
              <ul className="flex flex-wrap gap-2">
                {trade.emotionAfter.map((slug) => (
                  <li
                    key={slug}
                    className="border-accent/40 text-foreground bg-accent/10 inline-flex items-center rounded-full border px-3 py-1 text-xs"
                  >
                    {emotionLabel(slug)}
                  </li>
                ))}

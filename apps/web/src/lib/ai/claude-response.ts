...
/**
 * Catalogue des modèles Claude — ré-exporté depuis `./models` (la SSOT TS).
 * Conservé ici pour la compatibilité des imports existants
 * `from '@/lib/ai/claude-response'` ; toute NOUVELLE référence devrait importer
 * directement `./models`. La parité avec l'allowlist bash et le refine `env.ts`
 * est verrouillée par `models.parity.test.ts`.
 */
export { CLAUDE_LOCAL_SENTINEL, KNOWN_CLAUDE_MODEL_SLUGS } from './models';
...
/**
 * Contrat de retour CANONIQUE d'une génération Claude (chemin SDK live OU ingest
 * batch local), générique sur la charge utile validée `T`. Les types par
 * pipeline (`WeeklyReportGeneration`, `CalendarGeneration`, …) en satisfont déjà
 * structurellement la forme ; toute NOUVELLE génération (sessions 2 → 10) devrait
 * l'adopter pour parler un seul langage de retour. `cost` reste hors contrat
 * (spécifique à la table de pricing de chaque pipeline).
 */
export interface EngineResult<T> {
  /** Charge utile déjà parsée ET validée (Zod) par l'appelant. */
  readonly output: T;
  /** Consommation de tokens (zéro-remplie si la forme du payload dérive). */
  readonly usage: ClaudeUsage;
  /** Slug du modèle attribué, ou le sentinel `claude-code-local`. */
  readonly model: string;
  /** `true` si produit par le mock (aucun appel réseau réel). */

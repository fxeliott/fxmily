...
/**
 * Known executable Claude model slugs — the TS-side single source of truth
 * for wire-provided `model` attribution pins (mirror of the bash allowlist in
 * `ops/scripts/lib/claude-batch-core.sh` and the `env.ts` ANTHROPIC_MODEL
 * refine ; extend all three together when a model is added). Excludes the
 * `claude-code-local` sentinel on purpose (it is never an executable slug —
 * it marks "local Max binary, model unattributed").
 */
...
/**
 * Honest fallback for a wire-provided model that is NOT a known slug : the
 * content was generated by the local Max binary, exact model unattributed.
 * Mirror of `CLAUDE_CODE_LOCAL_MODEL` in the pricing tables.
 */

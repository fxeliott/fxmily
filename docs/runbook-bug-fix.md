...
- **Sentry plan gratuit** câblé côté serveur + client (J10 — pas avant). Tant que pas câblé, ce runbook est latent.
- **Source maps** uploadées en CI pour des stack traces lisibles (à wirer en J10).
- **Tags Sentry** : `userId` (anonymisé), `route`, `release_version`.
- **Repo GitHub privé** + branche `main` protégée + CI vert obligatoire pour merge.

# Historique des jalons — `apps/web`

Sorti de `apps/web/CLAUDE.md` le 2026-08-04 : ce fichier pesait 394 301 octets
(~104k tokens) et etait recharge en entier des qu'une session touchait `apps/web/`.
Ce qui suit est de la DOCUMENTATION datee — portee, fichiers touches, quality
gates, refs de PR. Les decisions qui restent OPPOSABLES en sont extraites vers
`.claude/rules/decisions-*.md`, ou elles se chargent quand on touche les fichiers
concernes. Rien n a ete supprime.
...
> Les decisions verrouillees de ce jalon vivent dans `.claude/rules/decisions-track.md` (regle path-scopee).
...
> Les decisions verrouillees de ce jalon vivent dans `.claude/rules/decisions-design-system.md` (regle path-scopee).
...
> Les decisions verrouillees de ce jalon vivent dans `.claude/rules/decisions-training.md` (regle path-scopee).
...
> Les decisions verrouillees de ce jalon vivent dans `.claude/rules/decisions-monthly-debrief.md` (regle path-scopee).
...
> Les decisions verrouillees de ce jalon vivent dans `.claude/rules/decisions-mindset.md` (regle path-scopee).
...
> Les decisions verrouillees de ce jalon vivent dans `.claude/rules/decisions-divers.md` (regle path-scopee).
...
> **⚠️ SUPERSEDED 2026-05-25 — PR [#172](https://github.com/fxeliott/fxmily/pull/172) (`68aee7f`)** : Track Record migré vers repo standalone [`trackrecord-fxmily`](https://github.com/fxeliott/trackrecord-fxmily) (vitrine CF Pages + admin Hono Worker + DB Neon Free). Les routes `/admin/track-record/*` + composants `components/admin/track-record/*` + helpers `lib/admin/public-trade-{math,service}` + schemas `lib/schemas/public-trade` + models Prisma `PublicTrade`/`PublicTradePartial` (gardés `@deprecated` pour sync DB Hetzner pré-drop) + 7 audit slugs `admin.public_trade.*` ont été RETIRÉS de `@fxmily/web` (PR #172 = -11204 lignes / 68 files / Vitest baseline 1601 → 1441). Section gardée intégralement pour traçabilité historique du build T0→T5 + canon Phase H/H+1/H+2/H+3/H+3.1/H+4/H+5/H+6/H+7/H+8 audit-driven hardening (pattern multi-rounds 5+ sub-agents post-impl réutilisable). **NE PAS supprimer cette section** — référence pédagogique du pattern Phase H+ pour futurs jalons code complex.
...
- **JSDoc `numFieldNullable` `Number.MAX_VALUE` boundary** (MED-4) — **shippé Phase H+3.1** commit `0f214f8` (`form-shapers.ts:67-87`).
...
- **Client-side Paris timezone consistency** (Phase H+8 — closes Phase H+5 round-trip) — `toDatetimeLocal` (pre-fill form) utilisait `d.getTimezoneOffset()` browser-local → admin en TZ ≠ Paris OU SSR runtime UTC produisait un wall-clock browser-dépendant que le server preprocess H+5 interpretait comme Paris → drift cumulatif silencieux. Fix : extracted dans `components/admin/track-record/datetime-paris.ts` (pure module testable) + `Intl.DateTimeFormat('en-CA', { timeZone: 'Europe/Paris' })` explicite. Symétrique avec server helper `parisLocalDatetimeToUtc`. **+8 tests TDD** (CEST + CET + Date object + null/undefined + invalid + day boundary + midnight collapse "24"→"00" + round-trip preserve UTC instant). Aussi : `formatDate` dans `public-trade-row.tsx` ajout `timeZone: 'Europe/Paris'` explicite (fix display "22 mai" au lieu de "23 mai" boundary pour trades Paris 1h AM = UTC 23:00 jour précédent). Cohérent SPEC §16 "Fuseau Europe/Paris".
...
- **`<PartialsSection>` edit-partial UI** (stress-test #2) — actuellement V1 ship create-only flow (admin add partial puis delete uniquement, pas d'edit). Si V1.x câble un edit-partial form, le `strField` ligne `actions.ts:282` doit devenir `strFieldNullable` pour permettre de clearer le commentaire d'une leg via update.
...
- **`dateTimeSchema` calendar validity check** (stress-test #7) — **shippé Phase H+7** commit `<HEAD>` : helper `parisLocalDatetimeToUtc` gate calendar validity (rejette month>12, day>31, day rolling Date.UTC) + `z.coerce.date({ error: () => "Date invalide." })` custom error sur new `Date(NaN)` retourné par le preprocess. +4 tests TDD (invalid month 13, invalid day Feb 30, Feb 29 non-leap 2026, Feb 29 leap 2024 accepté).
- **`setPublished` audit log spam idempotence** (api-designer YELLOW #6) — **shippé Phase H+7** : service retourne `{ row, wasChanged }`, action skip `logAudit` si `!wasChanged`. Toggle redondant n'émet plus d'audit row. +4 tests (publish redundant, publish first time, unpublish-then-published, unpublish redundant).
...
> Les decisions verrouillees de ce jalon vivent dans `.claude/rules/decisions-dashboard-a11y.md` (regle path-scopee).
...
> Les decisions verrouillees de ce jalon vivent dans `.claude/rules/decisions-pre-trade.md` (regle path-scopee).
...
> Les decisions verrouillees de ce jalon vivent dans `.claude/rules/decisions-onboarding.md` (regle path-scopee).
...
> **Jalon code safety-critical** post-recherche system card Opus 4.8 (réserve API détresse / means-substitution). Durcit le pipeline batch local Claude (`claude --print` Opus 4.8 qui analyse les réponses d'onboarding membre) sans toucher la logique métier. Reviews indépendantes code-reviewer + security-auditor = READY TO MERGE, 0 TIER 1/2.

### Scope (3 parties)

1. **`prompt.ts`** — nouveau bloc « SÉCURITÉ — DÉTRESSE / SIGNAUX DE CRISE » dans `ONBOARDING_INTERVIEW_SYSTEM_PROMPT` : interdit l'analyse/interprétation/citation des signaux de détresse, **aucune substitution de moyen**, route vers intervention humaine, pas d'interprétation émotionnelle/clinique non sollicitée, profil plus court plutôt qu'analyse de détresse. Anti-régression `prompt.test.ts` (4 assertions).
2. **`batch.ts`** — escalade `reportWarning` Sentry sur les gates `error` (refus Claude / exit non-nul) + `evidence_invalid` : tout entretien complété ne produisant AUCUN profil remonte pour review humaine (model-independent ; couvre résiduel over-refusal + drift modèle futur). Symétrique avec les escalades `amf_violation` / `clinical_language` existantes. + helper `canonicalizeBatchErrorCategory()` exporté.
3. **`docs/decisions/ADR-004`** — section datée « Safety hardening 2026-05-29 » + audit-trail entry.

> Les decisions verrouillees de ce jalon vivent dans `.claude/rules/decisions-onboarding.md` (regle path-scopee).

### Quality gate

- prettier ✓, lint 0 ✓, type-check 0 strict ✓, **Vitest 1618 → 1625** (+4 prompt anti-régression + 3 `canonicalizeBatchErrorCategory` PII boundary + 2 batch reportWarning) ✓, build prod Turbopack ✓.
- CI 6/6 GREEN + Deploy Hetzner SUCCESS (build 2m15s + SSH 23s, **0 migration**) + smoke prod `/api/health` ok/db/env 10:12Z.

### Reste (EN ATTENTE Eliot)
...
> Les decisions verrouillees de ce jalon vivent dans `.claude/rules/decisions-calendrier.md` (regle path-scopee).

...
#   bash ops/scripts/calendar-batch-local.sh --resume   # reuse /tmp/fxmily-calendar-batch/*.json
#                                                       # (skip pull, keep prior results, skip
#                                                       #  members already generated in that run)
#
# Required env :
...
# --resume keeps the prior run's NDJSON so already-generated members are never
# re-billed a claude call : each line is append-only-atomic, so every line that
# made it to disk is a complete, valid result. A fresh run truncates as before.
if [ "$RESUME" = "true" ] && [ -s "$RESULTS_NDJSON" ]; then
  echo "  --resume : keeping $(jq -s 'length' "$RESULTS_NDJSON") prior result line(s); already-generated members will be skipped."
else
  : >"$RESULTS_NDJSON"
fi
...
for idx in $ENTRY_INDICES; do
  ...
  # --resume : a member with a SUCCESS line already in the kept NDJSON was fully
  # generated by the interrupted run — skip (0 token). Error lines do NOT skip:
  # a failed member deserves the retry the resume exists for.
  if [ "$RESUME" = "true" ] && \
     jq -e -s --arg uid "$USER_ID" 'map(select(.userId == $uid and has("output"))) | length > 0' \
        "$RESULTS_NDJSON" >/dev/null 2>&1; then
    already_generated=$((already_generated + 1))
    echo "  [$i/$ENTRY_COUNT] $PSEUDO → SKIP (already generated by the interrupted run)"
    continue
  fi
  ...
  if [ $CLAUDE_EXIT -ne 0 ] || [ ! -s "$RESPONSE_FILE" ]; then
    ...
    if core_parse_response "$RESPONSE_FILE" "$PARSED_FILE"; then
      ...
      core_note_success
      # Incremental persist (best-effort) : push THIS member's calendar right
      # away, so a mid-run death (reboot, hard kill, quota halt) never strands
      # the members already generated until the next daily tick. The final
      # full persist below stays the idempotent net — re-upserting the same
      # calendar is harmless (generatedAt comes from snapshotTakenAt either
      # way). A failed incremental POST is logged and IGNORED on purpose.
      if [ "$DRY_RUN" = "false" ]; then
        jq -nc --arg ws "$WEEK_START" --arg sta "$RAN_AT" --arg uid "$USER_ID" \
           --slurpfile output "$PARSED_FILE" \
           '{weekStart: $ws, results: [{userId: $uid, output: $output[0]}]}
            + (if $sta == "" then {} else {snapshotTakenAt: $sta} end)' \
           >"$BATCH_DIR/incremental-$i.json"
        if core_persist_results "${APP_URL}/api/admin/calendar-batch/persist" \
                                "$FXMILY_CALENDAR_TOKEN" "$BATCH_DIR/incremental-$i.json" \
                                "$BATCH_DIR/incremental-persist-$i.json" >/dev/null 2>&1; then
          echo "    ✓ persisted incrementally"
        else
          echo "    ⚠ incremental persist failed (benign — the final persist below is the net)"
        fi
      fi
    else
      errored=$((errored + 1))
...
echo "  Generated: $generated, errored: $errored, skipped (no questionnaire): $skipped_no_questionnaire, already generated (resume): $already_generated"
...
# Gate on the NDJSON success count, NOT the loop counter : in --resume the kept
# lines from the interrupted run must still reach the final persist (its own
# persist step may be exactly what died).
SUCCESS_COUNT=$(jq -s 'map(select(has("output"))) | length' "$RESULTS_NDJSON")
if [ "$SUCCESS_COUNT" -eq 0 ]; then
  echo "[3/3] No calendars to persist (all errored or skipped)."
...
echo "[3/3] Persisting $SUCCESS_COUNT calendars to $APP_URL..."
if ! core_persist_results "${APP_URL}/api/admin/calendar-batch/persist" \

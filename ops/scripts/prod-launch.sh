...
#   - SSH config : alias `fxmily-prod` résoluble (cf. preflight-check.sh)
#
# Cycle complet (~15 min wall-clock dont ~10 min de polling DNS) :
#   1. preflight-check.sh        → SSH capacité fxmily-prod
...
if ((SKIP_PREFLIGHT == 0)); then
  step "Preflight check sur fxmily-prod"
  if ! bash "$SCRIPT_DIR/preflight-check.sh" "fxmily@fxmily-prod"; then
    cat <<EOF >&2

❌ Preflight a flag des problèmes. Options :
  A) Libère de la RAM/disque sur fxmily-prod et relance avec
...
echo "  → Set admin creds : echo -n 'NouveauMdp' | bash $SCRIPT_DIR/set-admin-creds.sh fxmily@fxmily-prod <admin-email>"
